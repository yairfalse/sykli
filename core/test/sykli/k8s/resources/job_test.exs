defmodule Sykli.K8s.Resources.JobTest do
  ...
  describe "create/3" do
    test "creates job from manifest" do
      ...
      result = Job.create(manifest, @config,
  ...
  describe "get/3" do
    test "gets job by name and namespace" do
      result = Job.get("my-job", "default", @config,
    ...
    test "returns not_found error when job doesn't exist" do
      result = Job.get("nonexistent", "default", @config,
  ...
  describe "delete/4" do
    test "deletes job with propagation policy" do
      result = Job.delete("my-job", "default", @config,
    ...
    test "returns not_found error when job doesn't exist" do
      result = Job.delete("nonexistent", "default", @config,
  ...
  describe "wait_complete/4" do
    test "returns :succeeded when job completes successfully" do
      ...
      result = Job.wait_complete("my-job", "default", @config,
        timeout: 5000,
        poll_interval: 10,
        client: fn :get, _path, nil, _config, _opts ->
          ...
          if count < 3 do
            ...
            {:ok, %{"status" => %{"succeeded" => 1, "conditions" => [
    ...
    test "returns :failed when job fails" do
      result = Job.wait_complete("failing-job", "default", @config,
        timeout: 5000,
        poll_interval: 10,
        client: fn :get, _path, nil, _config, _opts ->
          {:ok, %{"status" => %{"failed" => 1, "conditions" => [
    ...
    test "returns timeout error when job doesn't complete in time" do
      result = Job.wait_complete("slow-job", "default", @config,
        timeout: 50,
        poll_interval: 20,
        client: fn :get, _path, nil, _config, _opts ->
          {:ok, %{"status" => %{"active" => 1}}}
    ...
    test "returns error if job disappears during wait" do
      ...
      result = Job.wait_complete("disappearing-job", "default", @config,
        timeout: 5000,
        poll_interval: 10,
        client: fn :get, _path, nil, _config, _opts ->
          :counters.add(call_count, 1, 1)
  ...
  describe "logs/4" do
    test "gets logs from job's pod" do
      result = Job.logs("my-job", "default", @config,
    ...
    test "returns error when no pods found for job" do
      result = Job.logs("orphan-job", "default", @config,
    ...
    test "accepts container name option" do
      result = Job.logs("multi-container-job", "default", @config,
        container: "sidecar",
        pod_client: fn :get, path, nil, _config, _opts ->
          if String.contains?(path, "/pods") and not String.contains?(path, "/log") do
            {:ok, %{"items" => [
    ...
    test "handles pod not ready yet" do
      ...
      result = Job.logs("starting-job", "default", @config,
        retry_delay: 10,
        max_retries: 3,
        pod_client: fn :get, path, nil, _config, _opts ->
          :counters.add(call_count, 1, 1)
  ...
  describe "build_manifest/2" do
    test "builds minimal job manifest" do
      manifest = Job.build_manifest("test-job",
        image: "alpine:latest",
        command: ["echo", "hello"],
        namespace: "default"
      )
    ...
    test "adds labels to job" do
      manifest = Job.build_manifest("labeled-job",
        image: "alpine",
        command: ["true"],
        namespace: "default",
    ...
    test "adds environment variables" do
      manifest = Job.build_manifest("env-job",
        image: "alpine",
        command: ["printenv"],
        namespace: "default",
        env: %{"FOO" => "bar", "BAZ" => "qux"}
    ...
    test "adds volume mounts" do
      manifest = Job.build_manifest("mounted-job",
        image: "alpine",
        command: ["ls", "/data"],
        namespace: "default",
        volumes: [
    ...
    test "sets backoff limit" do
      manifest = Job.build_manifest("retry-job",
        image: "alpine",
        command: ["false"],
        namespace: "default",
        backoff_limit: 3
    ...
    test "sets TTL after finished" do
      manifest = Job.build_manifest("cleanup-job",
        image: "alpine",
        command: ["true"],
        namespace: "default",
        ttl_seconds: 60
